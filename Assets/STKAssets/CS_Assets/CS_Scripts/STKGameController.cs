...
namespace StackGameTemplate
{
	...
	{
        public Transform vuforiaHolder;
		...
		void Start()
		{
			...
			if ( towerObject )
			{
                ...
                sliceHeight = defaulttower.GetComponentInChildren<MeshRenderer>().bounds.size.y;

                Transform newtowerObject = Instantiate( defaulttower, towerObject.position + Vector3.up * sliceHeight, towerObject.rotation) as Transform;

                newtowerObject.localScale *= vuforiaHolder.localScale.x;
            ...
            if (highScoreObject)
            {
                highScoreObject.position = Vector3.up * (towerObject.position.y + sliceHeight * highScore);
		...
		public void CheckSlice()
		{
			...
			if ( moveDirection == 1 )
			{
				...
				if ( Mathf.Abs(towerObject.position.x - previoustowerObject.position.x) < minimumSliceSize )
				{
					...
					towerObject.position = new Vector3( previoustowerObject.position.x, previoustowerObject.position.y + sliceHeight, previoustowerObject.position.z);

                    this.transform.parent.position -= Vector3.up * sliceHeight;
					...
					{
						StartCoroutine(Expandtower( towerObject, new Vector3(Mathf.Clamp(towerObject.localScale.x + expandSize, 0, 1), sliceHeight, Mathf.Clamp(towerObject.localScale.z + expandSize, 0, 1))));
					...
					// Place the top tower perfectly on the one below it
					towerObject.position = new Vector3( towerObject.position.x, previoustowerObject.position.y + sliceHeight, towerObject.position.z);
                    ...
                    if (perfectEffect)
                    {
                        ...
                        newEffect.localScale *= vuforiaHolder.localScale.x;
                    ...
                    Transform nexttowerObject = Instantiate( previoustowerObject, previoustowerObject.position + Vector3.up * sliceHeight, previoustowerObject.rotation) as Transform;

                    //nexttowerObject.localScale *= vuforiaHolder.localScale.x;
				...
				else // If there is no perfect match and we also didn't completely miss the mark, slice the tower based on how much of it is above the tower below it
				{
                    ...
                    slicedtowerObject.localScale *= vuforiaHolder.localScale.x;
